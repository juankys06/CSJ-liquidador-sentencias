...
$(document).ready(function () {
    ...
    var t_liquidacion = $("#tab2 table").DataTable({
        "paging": false,
        "scrollX": true,
        "scrollY": "40vh",
        "scrollCollapse": true,
    ...
    var t_resumen = $("#tab3 table").DataTable({
        "paging": false,
        "ordering": false,
        "info": false,
        "searching": false,
                ...
                if (Array.isArray(json[element])) {
                    ...
                    if (element === "f_abono")
                        ...
                        for (let i = 0; i < fields_to_create; i++) {
                            ...
                            new AutoNumeric('#tab-tabla1 table tr:last-child .no-numbar', autoNumericOptionsThousands);
                        }
            ...
            if ($(this).val() != "") {
                if (new Date($(this).val()) < new Date($("input[name=f_exigibilidad]").val()) ||
                    ...
                    if ($("#err_msg").length == 0) //-- Dont exists
                        $('<span class="alert alert-danger" id="err_msg" style="font-size:0.602142em">Las fechas de las cuotas deben estar entre las fechas de exigibilidad y de liquidación</span>').insertAfter("#tab-tabla1 table");
    ...
    $('#tab-tabla1').on('focusout', 'input[name="cuota[]"]:last', function () {
        t_cuotas.row.add(['<i class="fa fa-trash delete"></i>', '<input type="date" name="f_cuota[]" max="2100-01-01" min="1900-01-01" />', '<input type="text"  class="no-numbar" name="cuota[]" />']).draw();
    ...
    $('#t_abonos').on('focusout', 'input[name="abono[]"]:last', function () {
        t_abonos.row.add(['<i class="fa fa-trash delete"></i>', '<input type="date" name="f_abono[]"  max="2100-01-01" min="1900-01-01" />', '<input type="text" class="no-numbar" name="abono[]" />']).draw();
        new AutoNumeric('#t_abonos tr:last-child td:last-child input', autoNumericOptionsThousands);
    ...
    $("#btn_add_ext").click( function () {
        ext_cuotas.row.add(['<i class="fa fa-trash delete"></i>','<input type="date" name="f_ext[]"  max="2100-01-01" min="1900-01-01" />', '<input type="text" class="no-numbar" name="ext[]" />', '<input type="checkbox" name="calcInt[]" />', '<input type="number" step=".00005" name="aSeguros[]" />']).draw();
        new AutoNumeric('#tab-tabla2 table tr:last-child .no-numbar', autoNumericOptionsThousands);
    ...
    $("#btn_add_multa").click(function () {
        t_multas.row.add(['<i class="fa fa-trash delete"></i>','<input type="date" name="f_multa[]"  max="2100-01-01" min="1900-01-01" />', '<input type="text" class="no-numbar" name="multa[]" />', '<input type="checkbox" name="intereses[]" />']).draw();
        new AutoNumeric('#tab-tabla3 table tr:last-child .no-numbar', autoNumericOptionsThousands);
    ...
    $('#btn_completar').click(function () {
        ...
        for (var i = 0; i < arreglo.length; i++) {
            t_cuotas.row.add(['<i class="fa fa-trash delete"></i>', '<input type="date"  max="2100-01-01" min="1900-01-01" name="f_cuota[]" value="' + arreglo[i].fecha + '" />', '<input type="text" class="no-numbar" name="cuota[]"  value="' + arreglo[i].monto + '" />']).draw();
        }
    ...
    $('#t_cuotas').on('focusout', 'input[name="f_cuota[]"]', function(){
        //-- Procesos.js
        validarAbonosCapitales($('input[name="f_exigibilidad"]'), $('input[name="f_liquidacion"]'), $(this));
    });
